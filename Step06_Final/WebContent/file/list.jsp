...
//한 페이지에 몇개씩 표시할 것인지
	...
	*/
	...
	if(keyword==null){
		...
		//클라이언트 웹브라우저에 출력할 때 "null"이 출력되지 않게하기 위해서
		keyword="";
		condition="";
	}
	...
	//전체 row 의 갯수를 담을 지역변수 미리 만든다.
	int totalRow=0;
	//만일 검색 키워드가 넘어온다면
	if(!keyword.equals("")){
		//검색 조건이 무엇이냐에 따라 분류
		if(condition.equals("title_filename")){//제목+파일명 검색
			//검색 키워드를 FileDto 에 담아서 전달
			dto.setTitle(keyword);
			dto.setOrgFileName(keyword);
			//제목+파일명 검색일때 호출하는 메소드를 이용해서 목록얻어오기
		...
		}//다른 검색 조건을 추가하고 싶다면 아래에 else if 를 계속 추가
		
	}else{//검색키워드가 넘어오지 않는다면
		//키워드가 없을때 호출하는 메소드를 이용해서 파일목록을 얻어온다
		list=FileDao.getInstance().getList(dto);
		//키워드가 없을때 호출하는 메소드를 이용해서 전체 row 갯수를 얻어온다
...
<div class="container">
	<a href="private/upload_form.jsp">업로드 하러가기</a><br />
	<a href="private/ajax_form.jsp">업로드 하러가기2</a>
	<h1>자료실 목록 입니다.</h1>
	<nav>
		<ul class="breadcrumb">
	...
	<a href="private/upload_form.jsp">자료 추가 하러가기</a>
